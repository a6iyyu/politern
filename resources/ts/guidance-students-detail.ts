...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (nama_mahasiswa == null || nim == null || prodi == null || nama_perusahaan == null || posisi == null || lokasi == null || ipk == null) return;
  ...
  buttons.forEach((button) => {
    button.addEventListener('click', async () => {
      ...
      try {
        ...
        if (logo_perusahaan && logo_placeholder) {
          const logo_path = data.magang.logo;

          if (logo_path) {
            const logo_url = logo_path.startsWith('/storage/')
              ? logo_path
              : logo_path.startsWith('storage/')
                ? `/${logo_path}`
                : `/storage/${logo_path}`;

            logo_perusahaan.src = logo_url;
            logo_perusahaan.classList.remove('hidden');
            logo_placeholder.classList.add('hidden');

            logo_perusahaan.onerror = () => {
              console.error('Gagal memuat logo: ', logo_url);
