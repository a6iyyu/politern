@if (isset($proyek) && $proyek->id_proyek > 0)
    <section class="modal modal-edit-proyek-mahasiswa fixed inset-0 z-50 hidden items-center justify-center bg-black/20 backdrop-blur-[1px]" aria-modal="true" role="dialog">
        <div class="w-full flex items-center justify-center min-h-screen px-4">
            <form action="{{ route('mahasiswa.profil.proyek.perbarui', ['id' => $proyek->id_proyek]) }}" method="POST" enctype="multipart/form-data" class="max-h-[90vh] overflow-y-auto w-9/10 rounded-xl bg-white px-7 py-5 shadow-lg border border-[var(--stroke)] md:w-3/4 lg:w-1/2">
                @csrf
                @method('PUT')
                <span class="mb-3 flex items-center justify-between">
                    ...
                    <i class="close-modal-project fa-solid fa-xmark cursor-pointer text-[var(--primary)]"></i>
                </span>
                ...
                <fieldset class="my-4 flex flex-col gap-1">
                    <label for="edit_deskripsi_proyek" class="text-sm font-medium text-[var(--primary-text)]">
                        Deskripsi <span class="text-red-500">*</span>
                    </label>
                    <textarea
                        id="deskripsi"
                        name="edit_deskripsi_proyek"
                ...
                <span class="my-4 grid grid-cols-1 gap-4 md:grid-cols-2">
                    <x-input
                        ...
                        label="Tanggal Mulai"
                        type="date"
                        name="edit_tanggal_mulai_proyek"
                        :required="true"
                    />
                    <x-input
                        icon="fa-solid fa-calendar"
                        ...
                        type="date"
                        name="edit_tanggal_selesai_proyek"
                        :required="true"
                    />
                </span>
