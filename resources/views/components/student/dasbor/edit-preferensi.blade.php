                ...
                @foreach ($fields as $field => $meta)
                    <section>
                        ...
                        <div
                            ...
                            @foreach ($meta['items'] as $item)
                                <label
                                    ...
                                    <input type="checkbox" name="{{ $field }}[]" value="{{ $item[$meta['key']] }}" class="w-4 h-4 text-[var(--primary)] border-gray-300 rounded focus:ring-[var(--primary)] focus:ring-2"
                        ...
                        <p class="text-xs text-gray-500 mt-1">
                            Pilih {{ str_replace('_', ' ', $field) }} yang {{ $field === 'keahlian' ? 'dikuasai' : 'diinginkan' }}
                        </p>
                    </section>
                @endforeach
            </div>
            <span class="flex justify-end mt-8">
                <button type="submit" class="cursor-pointer bg-[var(--primary)] text-white px-6 py-3 rounded-md text-sm hover:bg-[#5955b2]/90 transition-all duration-300">
                    Simpan Preferensi
                </button>
            </span>
        </form>
