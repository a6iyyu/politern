            ...
            <div class="mb-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="md:col-span-2">
                    ...
                    <div class="flex gap-2 flex-col sm:flex-row">
                        <select id="keahlian-select-edit" class="flex-1 border rounded-lg p-2">
                            <option value="">Pilih jenis keahlian magang</option>
                            @foreach($keahlian as $id => $nama)
                                <option value="{{ $id }}">{{ $nama }}</option>
                            @endforeach
                        </select>
                        <button type="button" id="tambah-keahlian-edit" class="w-full sm:w-auto bg-pink-400 text-white px-4 py-2 rounded hover:bg-pink-500 transition">
                            Tambah
                        </button>
                    </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const tambahBtn = document.getElementById('tambah-keahlian-edit');
    const badgeContainer = document.getElementById('badge-keahlian-edit');
    const inputContainer = document.getElementById('input-keahlian-edit');
    let selectedKeahlian = [];

    // Inisialisasi dari data lama
    @if(isset($lowongan) && $lowongan->keahlian)
        @foreach($lowongan->keahlian as $k)
            selectedKeahlian.push({id: '{{ $k->id_keahlian }}', nama: '{{ $k->nama_keahlian }}'});
        @endforeach
    @endif
