<a
    href="{{ route('admin.periode-magang.detail', ['id' => $periode->id_periode]) }}"
...
<a
    href="{{ route('admin.periode-magang.edit', ['id' => $periode->id_periode]) }}"
...
<form action="{{ route('admin.periode-magang.hapus', ['id' => $periode->id_periode]) }}" method="POST" class="inline-block">
    @csrf
    @method('DELETE')
    <button
        type="submit"
