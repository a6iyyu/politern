        ...
        <section class="cursor-default container mx-auto p-6 rounded-lg bg-white border border-[var(--stroke)]">
            ...
            <div class="mb-8">
                ...
                <x-table 
                    ...
                    :sortable="[]"
                    :rows="collect($debug['matriks_alternatif'])->map(fn($baris, $id) => [
                        $id,
            ...
            <div class="mb-8">
                ...
                <x-table 
                    :headers="array_merge(['ID Lowongan'], array_keys(reset($debug['matriks_normalisasi'])))"
                    :sortable="[]"
                    :rows="collect($debug['matriks_normalisasi'])->map(fn($baris, $id) => array_merge([$id], array_map(fn($v) => number_format($v, 4), $baris)))->toArray()"
                />
            </div>
            ...
            <div class="mb-8">
                ...
                <x-table 
                    :headers="array_merge(['ID Lowongan'], array_keys(reset($debug['matriks_terbobot'])))"
                    :sortable="[]"
                    :rows="collect($debug['matriks_terbobot'])->map(fn($baris, $id) => array_merge([$id], array_map(fn($v) => number_format($v, 4), $baris)))->toArray()"
                />
            </div>
            ...
            <div class="mb-8">
                ...
                <x-table 
                    ...
                    :sortable="[]"
                    :rows="collect($debug['solusi_ideal_positif'])->map(fn($nilai, $kol) => [
                        $kol,
