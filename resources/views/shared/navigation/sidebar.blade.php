<aside class="fixed z-30 hidden h-full flex-col space-y-4 border-r-2 border-[#6d6adc] p-6 shadow-2xl bg-[#5955b2] transition-all duration-300 ease-in-out lg:flex">
    ...
    <nav class="mb-4 mt-2 flex overflow-y-scroll h-full flex-1 flex-col gap-4">
        @if (Auth::check() && Auth::user()->tipe === 'ADMIN')
            ...
            <a href="{{ route('admin.program-studi') }}" class="flex items-center font-semibold pl-5 pr-7 py-3 {{ Request::is('admin/program-studi*') ? 'bg-white rounded-lg text-[#5955b2]' : 'text-white/75' }}">
                <img src="{{ Request::is('admin/program-studi*') ? asset('icons/program-studi-biru.svg') : asset('icons/program-studi-putih.svg') }}" alt="Dasbor" class="h-5 w-5" />
                <h4 class="ml-7 text-sm tracking-wider">Program Studi</h4>
            </a>
